...
public abstract class Packet<T> {
    ...
    public static Packet<ImageProxy> of(ImageProxy imageProxy, Exif exif, Size size, Rect rect, int i, Matrix matrix, CameraCaptureResult cameraCaptureResult) {
        if (imageProxy.getFormat() == 256) {
            C1124b.p(exif, "JPEG image must have Exif.");
        }
        return new AutoValue_Packet(imageProxy, exif, imageProxy.getFormat(), size, rect, i, matrix, cameraCaptureResult);
    }

    public static Packet<byte[]> of(byte[] bArr, Exif exif, int i, Size size, Rect rect, int i9, Matrix matrix, CameraCaptureResult cameraCaptureResult) {
        return new AutoValue_Packet(bArr, exif, i, size, rect, i9, matrix, cameraCaptureResult);
    }
}
