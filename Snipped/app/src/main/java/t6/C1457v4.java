package t6;

import android.util.SparseIntArray;
import com.luminary.mobile.R;

/* compiled from: FragmentNewCompanyAdditionalInformationBindingImpl.java */
/* renamed from: t6.v4, reason: case insensitive filesystem */
/* loaded from: classes2.dex */
public final class C1457v4 extends AbstractC1450u4 {

    /* renamed from: v, reason: collision with root package name */
    public static final SparseIntArray f29717v;

    /* renamed from: u, reason: collision with root package name */
    public long f29718u;

    static {
        SparseIntArray sparseIntArray = new SparseIntArray();
        f29717v = sparseIntArray;
        sparseIntArray.put(R.id.nestedScrollView, 1);
        sparseIntArray.put(R.id.turnoverInputLayout, 2);
        sparseIntArray.put(R.id.turnoverInputView, 3);
        sparseIntArray.put(R.id.countryInputLayout, 4);
        sparseIntArray.put(R.id.countryInputView, 5);
        sparseIntArray.put(R.id.dateInputLayout, 6);
        sparseIntArray.put(R.id.dateCodeInputView, 7);
        sparseIntArray.put(R.id.imageView6, 8);
        sparseIntArray.put(R.id.vat_registered_text, 9);
        sparseIntArray.put(R.id.rgCrypto, 10);
        sparseIntArray.put(R.id.rbYes, 11);
        sparseIntArray.put(R.id.rbNo, 12);
        sparseIntArray.put(R.id.button_done, 13);
        sparseIntArray.put(R.id.button_done_k, 14);
    }

    /* JADX WARN: Illegal instructions before constructor call */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
        To view partially-correct code enable 'Show inconsistent code' option in preferences
    */
    public C1457v4(F0.c r13, android.view.View r14) {
        /*
            r12 = this;
            android.util.SparseIntArray r0 = t6.C1457v4.f29717v
            r1 = 15
            java.lang.Object[] r0 = F0.f.h(r14, r1, r0)
            r1 = 13
            r1 = r0[r1]
            r5 = r1
            android.widget.Button r5 = (android.widget.Button) r5
            r1 = 14
            r1 = r0[r1]
            r6 = r1
            android.widget.Button r6 = (android.widget.Button) r6
            r1 = 4
            r1 = r0[r1]
            com.luminary.business.presentation.ui.views.customtextinput.CustomTextInputLayout r1 = (com.luminary.business.presentation.ui.views.customtextinput.CustomTextInputLayout) r1
            r1 = 5
            r1 = r0[r1]
            r7 = r1
            com.luminary.business.presentation.ui.views.customtextinput.CustomTextInputEditText r7 = (com.luminary.business.presentation.ui.views.customtextinput.CustomTextInputEditText) r7
            r1 = 7
            r1 = r0[r1]
            r8 = r1
            com.luminary.business.presentation.ui.views.customtextinput.CustomTextInputEditText r8 = (com.luminary.business.presentation.ui.views.customtextinput.CustomTextInputEditText) r8
            r1 = 6
            r1 = r0[r1]
            com.luminary.business.presentation.ui.views.customtextinput.CustomTextInputLayout r1 = (com.luminary.business.presentation.ui.views.customtextinput.CustomTextInputLayout) r1
            r1 = 8
            r1 = r0[r1]
            android.widget.ImageView r1 = (android.widget.ImageView) r1
            r1 = 1
            r1 = r0[r1]
            androidx.core.widget.NestedScrollView r1 = (androidx.core.widget.NestedScrollView) r1
            r1 = 12
            r1 = r0[r1]
            r9 = r1
            android.widget.RadioButton r9 = (android.widget.RadioButton) r9
            r1 = 11
            r1 = r0[r1]
            r10 = r1
            android.widget.RadioButton r10 = (android.widget.RadioButton) r10
            r1 = 10
            r1 = r0[r1]
            android.widget.RadioGroup r1 = (android.widget.RadioGroup) r1
            r1 = 2
            r1 = r0[r1]
            com.luminary.business.presentation.ui.views.customtextinput.CustomTextInputLayout r1 = (com.luminary.business.presentation.ui.views.customtextinput.CustomTextInputLayout) r1
            r1 = 3
            r1 = r0[r1]
            r11 = r1
            com.luminary.business.presentation.ui.views.customtextinput.CustomTextInputEditText r11 = (com.luminary.business.presentation.ui.views.customtextinput.CustomTextInputEditText) r11
            r1 = 9
            r1 = r0[r1]
            android.widget.TextView r1 = (android.widget.TextView) r1
            r2 = r12
            r3 = r13
            r4 = r14
            r2.<init>(r3, r4, r5, r6, r7, r8, r9, r10, r11)
            r1 = -1
            r12.f29718u = r1
            r13 = 0
            r13 = r0[r13]
            android.widget.LinearLayout r13 = (android.widget.LinearLayout) r13
            r0 = 0
            r13.setTag(r0)
            r12.l(r14)
            monitor-enter(r12)
            r13 = 1
            r12.f29718u = r13     // Catch: java.lang.Throwable -> L7c
            monitor-exit(r12)     // Catch: java.lang.Throwable -> L7c
            r12.i()
            return
        L7c:
            r13 = move-exception
            monitor-exit(r12)     // Catch: java.lang.Throwable -> L7c
            throw r13
        */
        throw new UnsupportedOperationException("Method not decompiled: t6.C1457v4.<init>(F0.c, android.view.View):void");
    }

    @Override // F0.f
    public final void b() {
        synchronized (this) {
            this.f29718u = 0L;
        }
    }

    @Override // F0.f
    public final boolean e() {
        synchronized (this) {
            try {
                return this.f29718u != 0;
            } catch (Throwable th) {
                throw th;
            }
        }
    }

    @Override // F0.f
    public final boolean m(int i, Object obj) {
        return true;
    }
}
