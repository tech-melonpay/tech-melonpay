...
final class DecodeHelper<Transcode> {
    ...
    public List<Key> getCacheKeys() {
        if (!this.isCacheKeysSet) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (int i9 = 0; i9 < loadData2.alternateKeys.size(); i9++) {
                    if (!this.cacheKeys.contains(loadData2.alternateKeys.get(i9))) {
                        this.cacheKeys.add(loadData2.alternateKeys.get(i9));
                    }
                }
            }
        }
    ...
    public <X> Encoder<X> getSourceEncoder(X x9) {
    ...
    public <Z> Transformation<Z> getTransformation(Class<Z> cls) {
        ...
        if (this.transformations.isEmpty() && this.isTransformationRequired) {
            throw new IllegalArgumentException(a.l("Missing transformation for ", cls, ". If you wish to ignore unknown resource types, use the optional transformation methods."));
        }
    ...
    /* JADX WARN: Multi-variable type inference failed */
    public <R> void init(GlideContext glideContext, Object obj, Key key, int i, int i9, DiskCacheStrategy diskCacheStrategy, Class<?> cls, Class<R> cls2, Priority priority, Options options, Map<Class<?>, Transformation<?>> map, boolean z10, boolean z11, DecodeJob.DiskCacheProvider diskCacheProvider) {
