...
public class JceX448Domain implements TlsECDomain {
    ...
    public PublicKey decodePublicKey(byte[] bArr) {
        try {
            return this.crypto.getHelper().createKeyFactory(XDHParameterSpec.X448).generatePublic(new X509EncodedKeySpec(new SubjectPublicKeyInfo(new AlgorithmIdentifier(EdECObjectIdentifiers.id_X448), bArr).getEncoded(ASN1Encoding.DER)));
        } catch (Exception e10) {
