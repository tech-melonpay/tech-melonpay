...
public class SecT163R1Point extends ECPoint.AbstractF2m {
    ...
    public ECPoint add(ECPoint eCPoint) {
        ...
        ECFieldElement eCFieldElement8 = this.f25200y;
        ...
        if (rawXCoord.isZero()) {
            ...
            eCFieldElement4 = a.t(divide, divide, xCoord).add(curve.getA());
    ...
    public ECPoint twice() {
        ...
        ECFieldElement t3 = a.t(eCFieldElement2, multiply, a10);
        if (t3.isZero()) {
            return new SecT163R1Point(curve, t3, curve.getB().sqrt());
        }
        ECFieldElement square2 = t3.square();
        ECFieldElement multiply2 = isOne ? t3 : t3.multiply(square);
        if (!isOne) {
            eCFieldElement = eCFieldElement.multiply(eCFieldElement3);
        }
        return new SecT163R1Point(curve, square2, eCFieldElement.squarePlusProduct(t3, multiply).add(square2).add(multiply2), new ECFieldElement[]{multiply2});
    }
