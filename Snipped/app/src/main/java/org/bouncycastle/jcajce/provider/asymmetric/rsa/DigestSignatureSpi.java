...
public class DigestSignatureSpi extends SignatureSpi {
    ...
    public void engineInitSign(PrivateKey privateKey) {
        if (!(privateKey instanceof RSAPrivateKey)) {
            throw new InvalidKeyException(a.n(new StringBuilder("Supplied key ("), getType(privateKey), ") is not a RSAPrivateKey instance"));
        }
    ...
    public void engineInitVerify(PublicKey publicKey) {
        if (!(publicKey instanceof RSAPublicKey)) {
            throw new InvalidKeyException(a.n(new StringBuilder("Supplied key ("), getType(publicKey), ") is not a RSAPublicKey instance"));
        }
    ...
    public boolean engineVerify(byte[] bArr) {
        ...
        byte b9 = (byte) (derEncode[3] - 2);
        derEncode[3] = b9;
        int i = b9 + 4;
        int i9 = b9 + 6;
        int i10 = 0;
        for (int i11 = 0; i11 < derEncode.length - i9; i11++) {
            i10 |= processBlock[i + i11] ^ derEncode[i9 + i11];
        }
