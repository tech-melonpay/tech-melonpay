...
public class SM2Engine {
    ...
    private byte[] decrypt(byte[] bArr, int i, int i9) {
        int i10;
        ...
        if (mode == mode2) {
            System.arraycopy(bArr, i + i11 + digestSize, bArr3, 0, i12);
        } else {
            System.arraycopy(bArr, i + i11, bArr3, 0, i12);
        }
        ...
        if (this.mode == mode2) {
            ...
            for (int i13 = 0; i13 != digestSize2; i13++) {
                i10 |= bArr4[i13] ^ bArr[(i + i11) + i13];
            }
        } else {
            i10 = 0;
            for (int i14 = 0; i14 != digestSize2; i14++) {
                i10 |= bArr4[i14] ^ bArr[a.u(i, i11, i12, i14)];
            }
        }
    ...
    private byte[] encrypt(byte[] bArr, int i, int i9) {
        byte[] encoded;
        ECPoint normalize;
    ...
    private void xor(byte[] bArr, byte[] bArr2, int i, int i9) {
    ...
    public int getOutputSize(int i) {
        return this.digest.getDigestSize() + (this.curveLength * 2) + 1 + i;
    }
    ...
    public byte[] processBlock(byte[] bArr, int i, int i9) {
        return this.forEncryption ? encrypt(bArr, i, i9) : decrypt(bArr, i, i9);
    }
