...
public class SM2KeyExchange {
    ...
    private byte[] kdf(ECPoint eCPoint, byte[] bArr, byte[] bArr2, int i) {
        ...
        while (i10 < i9) {
            ...
            i11++;
            Pack.intToBigEndian(i11, bArr3, 0);
            this.digest.update(bArr3, 0, 4);
            this.digest.doFinal(bArr3, 0);
            int min = Math.min(digestSize, i9 - i10);
    ...
    private BigInteger reduce(BigInteger bigInteger) {
        return bigInteger.and(BigInteger.valueOf(1L).shiftLeft(this.f24775w).subtract(BigInteger.valueOf(1L))).setBit(this.f24775w);
    }
    ...
    public byte[] calculateKey(int i, CipherParameters cipherParameters) {
        ...
        return this.initiator ? kdf(calculateU, z10, z11, i) : kdf(calculateU, z11, z10, i);
    }
    ...
    public byte[][] calculateKeyWithConfirmation(int i, byte[] bArr, CipherParameters cipherParameters) {
        ...
        ECPoint calculateU = calculateU(sM2KeyExchangePublicParameters);
        if (!this.initiator) {
            byte[] kdf = kdf(calculateU, z11, z10, i);
        ...
        byte[] kdf2 = kdf(calculateU, z10, z11, i);
    ...
    public void init(CipherParameters cipherParameters) {
        ...
        this.f24775w = (this.ecParams.getCurve().getFieldSize() / 2) - 1;
    }
