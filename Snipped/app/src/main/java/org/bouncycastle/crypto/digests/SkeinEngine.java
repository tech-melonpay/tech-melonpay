...
public class SkeinEngine implements Memoable {
    ...
    public class UBI {
        ...
        private void processBlock(long[] jArr) {
            ...
            for (int i9 = 0; i9 < jArr.length; i9++) {
                jArr[i9] = jArr[i9] ^ this.message[i9];
            }
        }
        ...
        public void update(byte[] bArr, int i, int i9, long[] jArr) {
            int i10 = 0;
            while (i9 > i10) {
                ...
                int min = Math.min(i9 - i10, this.currentBlock.length - this.currentOffset);
                System.arraycopy(bArr, i + i10, this.currentBlock, this.currentOffset, min);
                i10 += min;
    ...
    public SkeinEngine(int i, int i9) {
        this.singleByte = new byte[1];
        if (i9 % 8 != 0) {
            throw new IllegalArgumentException(a.g(i9, "Output size must be a multiple of 8 bits. :"));
        }
        this.outputSizeBytes = i9 / 8;
    ...
    private static void initialState(int i, int i9, long[] jArr) {
        INITIAL_STATES.put(variantIdentifier(i / 8, i9 / 8), jArr);
    }

    private void output(long j10, byte[] bArr, int i, int i9) {
        ...
        for (int i11 = 0; i11 < i10; i11++) {
            int i12 = i11 * 8;
            int min = Math.min(8, i9 - i12);
            if (min == 8) {
                ThreefishEngine.wordToBytes(jArr[i11], bArr, i12 + i);
            } else {
                ThreefishEngine.wordToBytes(jArr[i11], bArr2, 0);
                System.arraycopy(bArr2, 0, bArr, i12 + i, min);
            }
        }
    }
    ...
    private static void sort(Parameter[] parameterArr) {
        ...
        for (int i = 1; i < parameterArr.length; i++) {
            ...
            while (i9 > 0) {
                int i10 = i9 - 1;
                if (parameter.getType() < parameterArr[i10].getType()) {
                    parameterArr[i9] = parameterArr[i10];
                    i9 = i10;
                }
            }
            parameterArr[i9] = parameter;
        }
    }
    ...
    private static Integer variantIdentifier(int i, int i9) {
        return Integers.valueOf(i | (i9 << 16));
    }
    ...
    public int doFinal(byte[] bArr, int i) {
        ...
        int i10 = ((this.outputSizeBytes + blockSize) - 1) / blockSize;
        for (int i11 = 0; i11 < i10; i11++) {
            int i12 = i11 * blockSize;
            output(i11, bArr, i + i12, Math.min(blockSize, this.outputSizeBytes - i12));
        }
        reset();
    ...
    public void update(byte[] bArr, int i, int i9) {
        checkInitialised();
        this.ubi.update(bArr, i, i9, this.chain);
    }
